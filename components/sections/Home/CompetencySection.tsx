...
export default function CompetencySection() {
      ...
      <div className="flex flex-col gap-2 overflow-x-auto pb-2">
        {[0, 1].map((rowIdx) => (
          <ul key={rowIdx} className="flex gap-2 whitespace-nowrap">
            {item.competency.filter((_, i) => i % 2 === rowIdx).map((competency) => {
  ...
  return (
    <section id="competencies" className="relative p-2 py-4 sm:p-3 sm:py-4 h-screen bg-primary">
      <div className="h-full px-5 pt-5 sm:p-20 bg-background rounded-2xl sm:rounded-4xl">
        <AnimatedContent
